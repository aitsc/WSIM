...
//				updateEstimatedParameters(); //在 saveIteratedModel() 中执行
	...
	public void saveIteratedModel(int iters, Documents docSet) throws IOException {
		...
		//主题_词次数矩阵.txt K*V
		writer = new BufferedWriter(new FileWriter(resPath + modelName + "-主题_词次数矩阵.txt"));		
		for (int i = 0; i < K; i++){
			for (int j = 0; j < V; j++){
				writer.write(nkt[i][j] + "\t");
			}
			writer.write("\n");
		}
		writer.close();
		...
		//文档_主题次数矩阵.txt M*K
		writer = new BufferedWriter(new FileWriter(resPath + modelName + "-文档_主题次数矩阵.txt"));
		for(int i = 0; i < M; i++){
			for(int j = 0; j < K; j++){
				writer.write(nmk[i][j] + "\t");
			}
			writer.write("\n");
		}
		writer.close();
